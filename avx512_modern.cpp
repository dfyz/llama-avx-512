...
inline static void ggml_vec_dot_q4_0(const int n, float * __restrict__ s, const void * __restrict__ vx, const void * __restrict__ vy) {
    ...
    for (int i = 0; i < nb; i += 2) {
        ...
        const __mmask64 neg_mask = _mm512_movepi8_mask(bx);
        const __m512i abs_bx = _mm512_abs_epi8(bx);
        const __m512i neg_by = _mm512_mask_sub_epi8(by, neg_mask, _mm512_setzero_si512(), by);

        const __m512i prod = _mm512_dpbusds_epi32(
